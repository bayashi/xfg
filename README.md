...
* Files and directories whose names start with a dot are ignored by default
    * Use `-.` or `--hidden` option to search for hidden-files/directories
        ...
        * With `-n` or `--search-default-skip-stuff` option, you can search for above default skipping files and directories
            * `-n` is same behavior with `--hidden` and `--no-default-skip` option
